...
class som_downscale(object):
	...
	def fit(self, model_timeseries, obs_timeseries, seed = 1):
		#Because the SOM itself is an unsupervised method, a single som can be trained for multiple downscale locations that all fall in the same input data grid point. The differences are in the PDFs fit to each SOM node. Fit, in order to match the API of the other methods, both trains the SOM, and fits the PDF, but these steps can be broken out into multiple steps with the map and fit_pdf functions
		self.map(model_timeseries, seed = 1)
	...
	def map(self, model_timeseries, seed = 1):
		...
		with graph.as_default():
			...
			som = SOM(m=self.som_y, n=self.som_x, dim=dims, batch_size=self.batch, initial_learning_rate=self.alpha, graph=graph, session=session, input_tensor=nextElement, max_epochs=self.epochs)
	...
	def fit_pdfs(self, model_timeseries, obs_timeseries):
		for i in range(len(model_timeseries)):
			...
			min_index = min([j for j in range(len(self.output_weights))], key=lambda x: np.linalg.norm(dayVector - self.output_weights[x]))
	...
	def generate_obs(self, index, rank = None):
		"""
		generates a daily observation based on the given values. If rank is given, the observation will be taken at that quantile, otherwise it will be randomly generated.
		rank: rank of CDF to generate at. default to random if None
		:return: float value of observation
		"""
		if rank is None:
			rank = random.random()
		#following Crane and Hewitson (2006), create a spline fit CDF to the cluster values, and select the value at the specified (or random) rank value
		#CDF code adapted from: https://stackoverflow.com/questions/24575869/read-file-and-plot-cdf-in-python
	...
	def plot_nodes(self, weights_index = None, size_x = None, size_y = None, stdevs = None, means = None, fmt = matplotlib.ticker.FormatStrFormatter('%.0f'), cmap = cm.gist_ncar, **kwargs):
		"""
		plots the grid of SOM nodes, with contours for a selected range of weights, intended to correspond to a single variable over a spatial window. 
		:param weights_index:
		:param stdevs:
		:param means: 
		:param cmap:
	...
	def node_stats(self):
		"""
		Calculates information about the nodes of the SOM. SOM must be trained before running
		returns: average value of node, frequency of node in training data, perent of days with a value of 0 (mainly for precipitation, the number of dry days)
		"""
		n = self.som_x
		m = self.som_y
