...
def normalize_climate_data(model_data, means= None, stdevs = None):
	"""
	Takes the means and stdev for a historical/reanalysis data and calculates z-scores of the passed in climate data based on those. if means and standard deviations are given, calculates the z-scores from those values, rather than the model_data, so that the different periods of data can be compared
	If means and stdevs are None, scale to the mean and stdev of the data
	:param model_data: example shape - (10950x175) for 30 years with 7 variables on 5x5 grid
	:param means: shape should match all but first axis of model_data
	:param stdevs: shape same as means
	:return: scaled model_data; returns same stdevs and means if they are passed in, and the stdev and means of the cliamte data if none are passed in.
	"""
	if means is None:
...
def select_season(data, start_day, end_day):
	"""
	...
	"""
	if start_date >= 0:
		stationValues = [data[i] for i in range(len(data)) if
	...
	for i in range(len(data)):
		if (i-numLeapDays)%365 == 31+28 and flag:
			if (start_year+(i-numLeapDays)//365)%4 == 0 and (start_year+(i-numLeapDays)//365) %100 != 0:
				numLeapDays += 1
				flag = False
			elif (start_year + (i - numLeapDays) // 365) % 4 == 0 and (start_year + (i - numLeapDays) // 365) % 1000 == 0:
				numLeapDays += 1
				flag = False
			else:
